import Mobilescreen from "../assets/mobilescreen.png"

const AppDownloadSection = () => {
  return (
    <div className="bg-blue-900 min-h-[400px] w-full relative overflow-hidden">
      <div className="container mx-auto px-6 py-12 flex flex-col md:flex-row items-center justify-between">
        {/* Left Content */}
        <div className="max-w-md mb-12 md:mb-0">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">
            Manage Your Services by your Mobile Phone
          </h1>
          <p className="text-blue-100 mb-8">
            Download our app to manage and track your services. Our app enables you to stay in touch with our experts and aids you in tracking your progress.
          </p>
          <div className="flex flex-wrap gap-4">
            {/* App Store Button */}
            <button className="flex items-center bg-black text-white px-6 py-2 rounded-lg hover:bg-gray-800 transition-colors">
              <svg className="w-8 h-8 mr-2" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
              </svg>
              <div className="text-left">
                <div className="text-xs">Get it on</div>
                <div className="text-sm font-semibold">App Store</div>
              </div>
            </button>
            
            {/* Google Play Button */}
            <button className="flex items-center bg-black text-white px-6 py-2 rounded-lg hover:bg-gray-800 transition-colors">
              <svg className="w-8 h-8 mr-2" viewBox="0 0 24 24" fill="currentColor">
                <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.5,12.92 20.16,13.19L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z" />
              </svg>
              <div className="text-left">
                <div className="text-xs">Get it on</div>
                <div className="text-sm font-semibold">Google Play</div>
              </div>
            </button>
          </div>
        </div>

        {/* Right Content - Phone Mockups */}
        <div className="relative w-full max-w-md">
          <div className="relative z-10 transform translate-x-12 ">
            <div className="bg-black  rounded-3xl shadow-xl w-64 h-[523px]">
              <div className="w-full h-8 rounded-t-3xl relative">
                <div className="absolute top-2 left-1/2 transform -translate-x-1/2 w-16 h-1 bg-gray-600 rounded-full" />
              </div>
              <div className=" flex items-center justify-center h-[calc(100%-2rem)]">
                <img src={Mobilescreen} alt="RegisterKaro Logo" className="w-full" />
              </div>
            </div>
          </div>
          <div className="absolute top-12 right-12 z-0">
            <div className="bg-white rounded-3xl shadow-xl w-64 h-[500px]">
              <div className="bg-black  rounded-3xl shadow-xl w-64 h-[523px]">
                <div className="absolute top-2 left-1/2 transform -translate-x-1/2 w-16 h-1 bg-gray-600 rounded-full" />
              </div>
              <div className="p-4 flex items-center justify-center h-[calc(100%-2rem)]">
                <img src={Mobilescreen} alt="RegisterKaro Logo" className="w-full" />
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default AppDownloadSection;